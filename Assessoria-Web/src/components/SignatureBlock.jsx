...
export function SignatureBlock({ label, value, onChange, helper }) {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-4">
      ...
      {value && !drawing ? (
        <div className="flex h-44 items-center justify-center rounded-lg border border-dashed border-zinc-300 bg-zinc-50">
          <img src={value} alt={label} className="max-h-36 max-w-full object-contain" />
        </div>
      ) : (
        <div className="overflow-hidden rounded-lg border border-dashed border-zinc-300 bg-white">
          <SignatureCanvas
            ref={padRef}
            penColor="#18181b"
            onEnd={syncSignature}
